...
def preprocessing_dataset(cls, positive_threshold, beta, training, validation, train_batch_size, use_hard_negative, sampling_method):
    ...
    def bi_encoder(dialogue, true_candidate):
        predictions_candidate = cls.search_hard_negatives(anchor = 'dialogue', dialogue = dialogue, true_candidate = true_candidate, sampling_method = sampling_method , positive_threshold = positive_threshold, beta = beta, num_negatives = 1)
        count = increment()
        print(count)
    ...
    def contruct_training_instance(x):
        if use_hard_negative == False:
             negatives_intent = random.sample(candidates,1)[0]
             while negatives_intent == x['text2']:
                   negatives_intent = random.sample(candidates,1)[0]
             return InputExample(texts=[x['text1'], x['text2'], negatives_intent], label=float(x['label']))
        else:
             hard_negatives_intent = bi_encoder(x['text1'], x['text2'])[0]
             return InputExample(texts=[x['text1'], x['text2'], hard_negatives_intent], label=float(x['label']))
